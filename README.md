...
CodeA = unusedXP / 10 + 50 * levelcode + 50 * 15 * diff + 50 * 15 * 5 * medal4 + 50 * 15 * 5 * 2 * 
        ...
        (classcode + 8 * armourcode + 8 * 3 * trait + 8 * 3 * 18 * spec + 8 * 3 * 18 * 9 * normalizedtalent);
```
For short, it is:

```
CodeA = unusedXP / 10 + 50 * levelcode + 750 * diff + 3750 * medal4 + 7500 * medal5 + 15000 * medal6 + 
        60000 * (classcode + 8 * armourcode + 24 * trait + 432 * spec + 3888 * normalizedtalent);
```
...
It's no doubt that the *trait*, *spec* are **the Trait Code** and **the Spec Code**.
...
[Figure](http://i2.tietuku.com/88bb5969430cc986.png)
